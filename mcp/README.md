...
- Filters issues where `issues.persona` matches any entry in `roles` (case-insensitive, `ILIKE ANY`).
- Exception: if `"admin"` is present anywhere in `roles`, the persona filter is omitted entirely — all of that customer's issues are returned regardless of persona.
- An empty `roles` list matches nothing (fails closed).
- Unknown `customer_name` naturally returns `[]`.

Each returned issue includes: `id`, `customer_name`, `title`, `description`, `status`, `persona`, `reporter`, `last_updated_by`, `created_at`, `updated_at`.
...
async with Client("http://localhost:9000/mcp") as client:
    result = await client.call_tool(
        "retrieve_issue_updates",
        {"issue_id": 1, "roles": ["sales"]},
    )
    print(result.data)
```
...
- **Tool returns `[]`/`null` unexpectedly** — check that `issues.persona` actually matches one of the `roles` strings being passed (case-insensitive `ILIKE ANY`); seed data (`postgres/initv2.sql`) sets `persona` to `sales`, `operations`, or `support` per issue, and user roles are seeded 1:1 with those (`bob`→sales, `charlie`→operations, `tony`→support, `alice`→admin).
