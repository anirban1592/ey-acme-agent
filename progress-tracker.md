...
- [x] **4.11 Typed WebSocket response contract**
  ...
  **UI overhaul (2026-07-20)**: full frontend visual rework — dark theme, branded header ("Acme Support Companion" + username + logout), a left sidebar of past conversations with a "New chat" flow, and a waiting/thinking indicator while the agent is processing — all pure UI work on top of the existing `useChatSocket.ts` contract, which was **not modified** (verified via `git diff --stat`). New `frontend/src/conversationStore.ts` persists per-thread transcripts client-side in `localStorage` (the backend has no history-fetch endpoint, and Redis/LangGraph memory from Phase 4.10 is server-side agent context only, never replayed to the client). Thread switching works by writing the target `thread_id` into the same `chat_thread_id` cookie the hook already reads at mount, then remounting the hook-calling component via `key={activeThreadId}` — no hook code changed. New components: `Header.tsx`, `Sidebar.tsx`, `WaitingIndicator.tsx`, `ChatWindow.tsx` (extracted from `Chat.tsx`'s old body); `Chat.tsx` itself became the layout orchestrator. Styling is a new plain-CSS `frontend/src/styles/theme.css` (CSS custom properties, no new framework dependency), with Fraunces/Inter/IBM Plex Mono loaded via a Google Fonts `<link>` in `index.html`. Verified end-to-end in a real browser (Playwright) against the live stack: login, new-chat greeting (confirmed client-only, never sent over the WebSocket), a real agent reply rendering correctly in dark theme, and thread-switching restoring a prior transcript instantly from `localStorage` while the cookie/active-thread state stayed in sync. `npm run build` (`tsc && vite build`) passes with zero errors.
